...
carrier is in fact isomorphic to our na\"ive subset formulation of the refinement of $\mu F$ by
$\alpha$. In this section, we will sketch the proof of these facts, referring the reader to the
paper by Atkey et al. \citep{atkey2011refining} for further details.

\subsection{Preliminaries}

We begin with a few auxiliary definitions we will require for our proofs. First, TODO
...
fiber over the codomain of $f$, and vice versa.
...
to this category of algebras. Mirroring our construction of $F^\alpha$ itself, we will do this in
two stages: first, we will consider algebras over $\hat F : \FamSet \to \FamSet$, and then we will
limit our attention to an ``$\alpha$-relevant restriction'' thereof.
...
It is worth pausing to clarify why this definition type-checks: recall that $\hat F$ is an
endofunctor on \emph{families}, so $P$ is a pair of an indexing set and a family thereupon; $k$
is a morphism of families, which is a pair of functions; and $f$ is an algebra morphism over an
...
correspondingly restrict the category $\Alg_{\hat F}$ to the corresponding \emph{fiber} (defined
analogously to how we defined fibers over the families fibration).
...
In fact, it turns out that this is the same category as $\Alg_{F^\alpha}$, just
``decomposed'' in terms of $\hat F$ and $\alpha$. TODO: expand a little

\begin{lemma}
  Let $(A, \alpha : F A \to A)$ be an $F$-algebra. Then there is an isomorphism of categories
...
over the families fibration to $(\Alg_{\hat F})_\alpha$. 
