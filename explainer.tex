...
define the \emph{recursor} for the Boolean type, canonically known as \texttt{if}:\footnote{We will
adopt the convention of writing recursors with their \emph{major premise} (i.e., the value being
...
relevant definitions acquire some bureaucratic machinery when abstracted that we prefer not to
deal with.}
...
\section{Fibrational Constructions}

\subsection{Another View of Families}

For several of the constructions we give in this chapter, it will be helpful to have in mind an
...
transformation $\pi : \{-\} \Rightarrow U$ whose morphisms project the index from an element of an
indexed family:
\[ \pi_{(A,P)}(a, p) = a \]

The notation $\pi$ is suggestive of the fact that these morphisms are just projections of the first
...
\emph{truth-preserving} lifting---precise: to characterize the objects in the codomain of $F$ as
...
The construction\footnote{The motivation for this construction is largely found in its more general
form in the setting of fibrational category theory, which is developed on page 347 of
\citep{ghani2010fibrational}.}
