...
A state $\sigma:\text{Var} \to \text{Val}$ maps variables to values. Two states $\sigma,\sigma'$ are \textit{$\ell$-equivalent}, denoted $\sigma \sim_\ell \sigma'$, when for all $\ell'\leq \ell$, if $L(x) = \ell'$, then $\sigma(x) = \sigma'(x)$.
...
\end{definition}
Let \textbf{Rel} be the category whose objects are sets and whose morphisms $A \sim_R B$ are relations $\sim_R \subseteq A \times B$. We can think of $\ell$-equivalence as a morphism in \textbf{Rel}, where $\sigma \sim_\ell \sigma'$ means $(\sigma,\sigma') \in\;\sim_\ell\;\subseteq \Sigma \times \Sigma'$ for $\Sigma,\Sigma'$ sets of states. Since set functions $f:A \to B$ can be represented as graphs $f\;\subseteq A\times B$, where $(a,b) \in f$ if $f(a) = b$, we can consider set functions to be relations as well, so $\alpha:\Sigma \to \Sigma'$ is also a morphism in \textbf{Rel}. 
...
and the orange are elements of the lower set generated by $60$.
...
In the sheaf semantics chapter we defined a \textit{coverage}, which was defined to be a relation $T$ between families of morphisms with common codomain $c$ and objects $c$ of $\calC$. If a family $\{f_i: c_i \to c\}$ and object $c$ are in $T$, then $\{f_i : c_i \to c\}$ is a $T$-cover of $c$. Conditions for $T$ can be found in the aforemantioned chapter. A general intuition for these families of morphisms is to think of them as a way to partition an object $c$ by images of $f_i$. 
...
\marginnote{A monad $T$ is idempotent when $T^2 = T$ (i.e. $\mu = id$).}
...
\end{definition}
Luckily for us, the only continuous maps we care about in this chapter are inclusion maps. When $X \subseteq Y$ is open any open $U \subseteq X$ is open in $Y$ as well. $F(U)$ would be the colimit of $F(V)$ where $V \supseteq i(U) = U$, because $U$ is clearly the greatest lower bound (limit) on sets containing $U$, so dually $F(U)$ would be the least upper bound (colimit) on $F(V)$ with $V \supseteq U$. Then $i^*F(U) = F(U)$, and of course $F$ is a sheaf so $i^*F(U)$ automatically satisfies sheaf conditions for open $U$. 
...
Pushouts are dual to pullbacks, but it is not very intuitive to get a sense of what that means exactly. We can try to work it out by staring at the diagram:
...
In \textbf{Set}, $P$ is a "quotient of the disjoint union of $A$ and $B$", which can also be written $A\sqcup B/\sim$, where $\sim$ is the equivalence relation detailed above, i.e. $b \sim c$ if $\exists\; a\in A$ such that $f(a) = b, g(a) = c$. The pushout essentially collapses the images of $f$ and $g$ to equivalence classes in $P$. We provide a couple of examples in \textbf{Set}. 
...
\begin{definition}
    \begin{itemize}
        ...
        \end{itemize}
        \item A \textit{security policy} U is a lower set in $\mathcal{P}$. The notation $U \mid\vdash x$ is \\pronounced "U permits $x$", and means $U \cap x \neq \emptyset$. 
        \begin{itemize}
...
\begin{proposition}
    ...
    \begin{proof}
        The set of filters on $\calP$, ordered by inclusion, forms a poset. Once we show the sets $O_U := \{x \mid U \mid\vdash x\}$ are upper sets of the filters, we are done.
...
\begin{exercise}
    \label{ex2e}
    With the definition of the Alexandroff topology, we know these \\sets are open because they are upper sets. Prove/check they are \\open using the openness axioms instead.
...
corresponding open subspace of $P$, $(\{x \mid \angled{\ell} \mid\vdash x\})$ is denoted $P_{\angled{\ell}}$. The complement, $P \setminus P_{\angled{\ell}}$ is denoted $P_{\bullet \angled{\ell}}$.
...
\begin{lemma}
    ...
    We know each $P_{\angled{\ell}} = \{x \mid \angled{\ell} \mid\vdash x\}$ are open by definition. 
    ...
    \[\bigcup_{\ell \in \calP}\{x | \ell \in x\} = \bigcup_{\ell \in \calP}\{x \mid \angled{\ell} \mid\vdash x\} = O_U,\]
