...
$\gamma(a)$ is the largest $c \in \calC$ 
  ...
  that (1) implies (2) and (3).
  Then we'll prove that (2) and (3) both imply (4).
  Finally, we'll prove that (4) implies (1),
  giving the following strongly connected graph:
  \[% https://tikzcd.yichuanshen.de/#N4Igdg9gJgpgziAXAbVABwnAlgFyxMJZARgBoAGAXVJADcBDAGwFcYkRiQBfU9TXfIRTlSxanSat2AJm68QGbHgJFpo8QxZtEIAMxy+SwUTLSNk7SAAs3cTCgBzeEVAAzAE4QAtkhEgcEEjEPG6ePoh+AUjSISAe3kE0UYi6sfHhav6BKWlhSLpJ2eRclFxAA
\begin{tikzcd}[sep=tiny]
             & 1 \arrow[ld] \arrow[rd] &              \\
2 \arrow[rd] &                         & 3 \arrow[ld] \\
             & 4 \arrow[uu]            &
\end{tikzcd}\]

  \item (1) implies (3): Suppose $\alpha \dashv \gamma$.
    Then by definition $\alpha(\gamma(a)) \le a$.
    So it only remains to show that \(\gamma(a)\)
    is the largest \(c\) satisfying \(\alpha(c) \le a\).
  ...
  \item (1) implies (2): analogous to the previous case.

  \item Proof that (2) implies (4).

    Suppose that for any $c \in \calC$, $\alpha(c)$ is the
    smallest $a \in \calA$ such that $c \le \gamma(a)$.
    ...
    Since \(\alpha(c)\) is the smallest \(a'\)
    such that \(c \le \gamma(a')\),
    and \(a\) is one such \(a'\),
    it must be that
    \(\alpha(c) \le a\).
...
\item
    Define some helpful notation: the sets $c \downarrow \gamma = \{a \mid c \le \gamma(a) \}$
    and \(\alpha \downarrow a = \{c \mid \alpha(c) \le a\}\).
    Then the previous theorem can be re-expressed as
    \begin{align*}
    &\alpha\dashv\gamma\\
    \iff &(\forall c.\, \alpha(c) = \min (c \downarrow \gamma))\\
    \iff &(\forall a.\, \gamma(c) = \max (\alpha \downarrow a))\\
    \iff &(\forall a\,c.\, \alpha(c)\le a \iff c\le \gamma(a)).
    \end{align*}
\end{itemize}
...
\begin{itemize}
  ...
  and explains the notation we chose for the
  sets \(c\downarrow\gamma\)
  and \(\alpha\downarrow a\)
