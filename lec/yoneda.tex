...
\begin{itemize}
  ...
  \begin{align*}
  (F \times G)(\id_X) &= (x, y) \mapsto (\id_{F(X)}(x), \id_{G(X)}(y)) \\
  &=(x, y) \mapsto  \id_{F(X) \times G(Y)} (x, y).
  \end{align*}
  \item \emph{Preserves composition}: For morphisms $X \mor{f} Y \mor{g} Z$:
  \begin{fullwidth}
   \begin{align*}
    (F \times G )(g \circ f) 
    &= (F(g \circ f), G(g \circ f)) & \text{Def} \\
    &= \big(F(f) \circ F(g), G(f) \circ G(g)\big) & \text{Functoriality of $F$ and $G$} \\
    &= \big[ (F \times G)(f) \big] \circ \big[ (F \times G) (g)\big] & \text{Def}
  \end{align*}
...
Let $(a,b) \in (F \times G)(Y)$. Then, $\big[F(f) \circ \pi_{1,Y}\big](a,b)  = f(a)$.
For the other path, we have $\big[(F \times G)(f) \big](a, b) = (f(a), (f(b)))$, 
so clearly $\pi_{1,X} \circ \big[(F \times G)(f) \big](a, b) = f(a)$.
...
with $\calC$-indexed functions $\Gamma \mor{f} F$ and $\Gamma \mor{g} G$. 
Then, we must show there exists a unique $\calC$-indexed set $\Gamma \mor{\langle f, g \rangle}
...
\begin{align*}
  ...
  &= x \mapsto \Big(f_X(x), g_X(x)\Big)
\end{align*}

Then we have, for any indexing object $X$:
\begin{align*}
...
Now all that remains is to show uniqueness of $\langle f, g \rangle$. 
...
The crucial idea of the Yoneda lemma is that a copy of $\calC$ lives 
inside of its corresponding $\calC$-indexed category.
This lets ideas,
definitions, and intuitions from the $\calC$-indexed category 
-- which, as we've already described, has fantastically rich structure, including products, exponentials, etc. -- 
