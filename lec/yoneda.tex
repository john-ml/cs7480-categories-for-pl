...
\begin{enumerate}
    ...
    \begin{itemize}
        \item Recall from proof in class : given a $p\in PX,$ we defined
...
\begin{definition}
    ...
    \begin{enumerate}
        ...
        the points of $PX$.
    \end{enumerate}
    Definitions of composition and identity follow immediately from the definitions in $\cat,$ with the additional restriction of point preservation. It is relatively simple to show the desired categorical properties hold.
\end{definition}
The following proposition is actually in the course notes; I don't think it was mentioned in class. But it's useful to have.
\begin{proposition}
    Suppose $\cat-$indexed set $P$ is representable by $X$. There exists a $u\in PX$ called the \textit{P-universal element} which satisfies the following universal property : for any object $\Gamma$ of $\cat$ and any $g\in P\Gamma,$ there exists a unique $\hat{g}:\Gamma\to X$ such that $g = u \cdot_P \hat{g}$. Alternatively, $g = (P\hat{g})(u)$.
\end{proposition}
This next proposition was actually mentioned in class, but we can decipher it better with the above information:
\begin{proposition}
    A $\cat-$indexed set $P$ is representable if $P\cong\yo X$ for some object $X$ of $\cat$. Suppose $P$ is representable. Then there exists a \textit{universal element} $u\in PX$ and $u$ is a terminal object in the \textit{category of elements} of $P$.
\end{proposition}

We have from Proposition 1.3 the definition of the universal property that we
...
What does it mean for $u \in PX$ to be terminal in the category of elements of
...
Proposition 1.3 -- we can "factor" any $p\in PA$ into $f$ and $u$.


How do we get Proposition 1.4 from the Yoneda Lemma? These are the key ideas:
\begin{itemize}
...
\newthought{First we construct} $func$.
...
As usual we begin by gaining an intuition for what functors look like 
