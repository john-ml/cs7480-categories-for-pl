...
the statement and proof of the Yoneda lemma.
...
Concretely, this means that if any input \(f\) as shown in the picture above is changed into \(f \circ s\) via some morphism
$s : \Gamma' \to \Gamma$,
...
\end{align*}
Finally, the $\beta$ and $\eta$ laws become equations between nested applications of the functions \(\mathsf{Fst}\), \(\mathsf{Snd}\),
and \(\mathsf{Pair}\).
\begin{align*}
  \inferrule
    {\Gamma \vdash M : A \quad
...
which can be thought of as a structure-preserving map 
between categories.}
...
\begin{itemize}
  ...
  \item Next, let's explore what $\calC$-indexed sets look like on a more interesting 
...
\begin{center}
  ...
  \draw (-1,-1) circle [x radius=5mm,y radius=7mm];
  \draw (1,-1) circle [x radius=5mm,y radius=7mm];

  \draw[->] (d) to node[midway,above] {$F(f)$} (a);
  \draw[->] (c) to (a);
  \draw[->] (X) to node[auto,swap] {$f$} (Y);
  \draw [->,loop,distance=1.5em,in=305,out=235] (X) to (X);
  \draw [->,loop,distance=1.5em,in=305,out=235] (Y) to (Y);
  \end{tikzpicture}
\end{center}
...
exercise. Note that there are 4 possible choices of $\alpha$ here, one for each possible 
...
them in (\ref{cat:2obj}). We can visualize the data 
...
Do not worry; the original definition of category can be adjusted to account 
...
From this, we can observe that there is a single morphism $\yo Y \to \yo X$ 
...
ought to be the ways of shifting viewpoints from $A$ to $B$,
i.e.:
...
and $B$ internally if and only if it behaves like a product externally.
...
\begin{align*}
  ...
  (\yo Z)(X) = \{g \circ f\}
  \\
  (\yo Z)(f) = g \mapsto g \circ f \quad
  (\yo Z)(g) = \id_Z \mapsto g
\end{align*}
...
natural numbers mod 3 under addition, the category $\mathbb{Z}_3$. This category has a single object $\star$ and 
3 morphisms: the identity morphism $0$, and morphisms $1$ and  $2$, 
together with equations stipulating that $1 \circ 1 = 2, 1 \circ 2 = \id, 2 \circ 2 = 1, 2 \circ 1 = \id$.
...
Indeed it does! Observe: by naturality, we have that $P(1) \circ \alpha = \alpha \circ (\yo \star)(1)$.
This gives us a system of constraints that uniquely defines $\alpha$. Observe:
\begin{align*}
  & ((P(1) \circ \alpha))(0) = b = (\alpha \circ (\yo \star)(1))(0) \\ 
  & \Rightarrow b = \alpha(((\yo \star)(1))(0)) \\
  & \Rightarrow b = \alpha(1).
\end{align*}
A similar chain of logic lets us derive $\alpha(2) = c$. Notice what is happening 
here: $P$ is a functor, so it must ``behave like the monoid'': i.e., 
it has the same equations and morphism compositions. Here, the choice of 
$\alpha(0)$ is \emph{picking an element of $P$ that behaves like the identity}. 

Now, how do we see that there are exactly 4 natural transformations $\alpha :
\yo \star \Rightarrow P$? Let's look at $P$. Since $P$ is a functor, it has
copies of the $\mathbb{Z}_3$ in its codomain. We can see two copies: the
$\{a,b,c\}$ subset is an exact copy -- the elements $a,b,$ and $c$ correspond to
distinct numbers, and $\alpha$ will pick an arbitrary 0 point -- and the $\{d\}$
subset is a denerate copy with only a single number $0$.
So, there are 3 natural transformations that send $0$ to $\{a, b, c\}$, 
each picking a different 0 point, and 1 natural transformation into $\{d\}$
that sends all elements of $(\yo \star)(\star)$ to $d$.



% $\alpha_1(1) = b$, which 
% we can see by naturality: $P(1)\circ\alpha_1$. This gives us one more piece of data 
% about $\alpha_1$. Now, what about $\alpha_1(2)$? This is also forced: we learned that 
% $\alpha_1(1) = b$, so by naturality we have $(P(1) \circ \alpha_1)()$

% Another way of understanding what is going on is that, by functoriality $P$ is
% ``a copy of the monoid upstairs'': functoriality says that $P$ must behave 
% like some number of copies of the monoid. 

\section{Proving the baby Yoneda lemma}
\marginnote{Thanks to Bex Golovanov for the scribe notes for the next two subsections.
up a bit.}
...
\begin{example}[Category of elements of the terminal presheaf]
  For any category $\calC$, the presheaf category on $\calC$ has a 
  terminal object $\mathbf{1}$ defined:
  \begin{align*}
    \mathbf{1}(X) &= \{\star\} \\
    \mathbf{1}(X \mor{h} Y) &= \star \mapsto \star
  \end{align*}
  The category of elements $\int \mathbf{1}$ has objects $(X, p)$  
  where $p = \star$ and $X$ are objects of $\calC$, 
  and has morphisms $(X, p) \mor{h} (Y, q)$ where $F(h)(p) = q$; 
  this point preservation is clearly trivially satisfied since 
  $p = q = \star$. So, we can conclude that $\int \mathbf{1} = \calC$,
  i.e., the category of elements of the terminal object in $\calC$ 
  is the original category $\calC$.
\end{example}
...
% not sure here, had trouble keeping up a bit
\begin{center}
  \begin{tikzcd}
E \arrow[r, "h"] & A \arrow[r, "g"', shift right] \arrow[r, "f", shift left] & B
\end{tikzcd}
\end{center}
...
\begin{center}
  \begin{tikzcd}
Q \arrow[r, "h"] & A \arrow[r, "g"', shift right] \arrow[r, "f", shift left] & B
\end{tikzcd}
\end{center}
...
\begin{center}
  \begin{tikzcd}
R \arrow[r, "h"] & A \arrow[r, "g"', shift right] \arrow[r, "f", shift left] & B \\
Q \arrow[u, "f"] \arrow[ur]
\end{tikzcd}
\end{center}
...
Such a set is called a \emph{Grothendieck universe}.
We won't give the formal definition here; it's not essential
to understand the details.\footnote{%
If you're curious, check out the
\href{https://ncatlab.org/nlab/show/Grothendieck+universe}{nLab page}
for Grothendieck universe.}
For our purposes, all that matters is that
...
\item On the other side there are sets \(X\) such that \(X \notin \calU\). These sets are ``too large to fit'', so called \emph{\(\calU\)-large}.
\end{itemize}

The key point is that, while we cannot define a category of \emph{all} sets, we can define a category of \emph{\(\calU\)-small} ones.
...
so long as we take \(\Set\) to mean the category of \(\calU\)-small sets,
and \(\calC\) is locally \(\calU\)-small.
\begin{definition}[category of \(\calC\)-indexed sets]
