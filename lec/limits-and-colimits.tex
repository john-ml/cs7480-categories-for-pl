...
\begin{definition}[Morphism of cones]
  ...
  is a morphism \(f : P \to Q\) such that \(p_i = f \circ q_i\)
...
\begin{definition}[Limit preservation]
  \sloppy
  Let $\calC$ and $\calD$ be categories, let $F : \calD \to \calD$,
  and let $D : \calI \to \calC$ 
  be a diagram in $\calC$. 
  Suppose $D$ has a limit $X$ in $\calC$. Then,
  the functor $F$ \emph{preserves the limit of $D$}
  if
...
\begin{proposition}
  Any full and faithful functor preserves limits.
