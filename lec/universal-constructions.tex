...
Our development here will mirror the development so far of 
product types and unit types: we will identify a 
special object in a category, characterized by some morphism equalities
and a universal property,
such that it validates the equational theory of $\lambda$-expressions.
...
\begin{align}
  \dbracket{\Gamma \vdash \pllam{x : A}{M} : B} : \dbracket{\Gamma} \to B^A
\end{align}
We are gathering clues: the above tells us that there must exist some 
morphism $\dbracket{\Gamma}$ into the special object $B^A$. 
...
We can imagine this morphism as ``computing the closure'' of the lambda.
Next, we want to be able to interpret function application, which, following the 
mantra of denotational semantics, should be given as a composition of 
morphisms:
\begin{align}
  \dbracket{\Gamma \vdash M~N : B}
  = \dbracket{M} \circ \dbracket{N}
\end{align}
...
Let's break this definition down diagrammatically and go through it 
step-by-step. 
First, let $A$ and $B$ be objects in our category; when thought of as 
types, these objects will denote the domain and codomain of the
lambda. Then, we can visualize $A$ and $B$ together with their 
exponential object $B^A$ and application function $\app{}$ as:

Categories with exponents get a special name: they are called
\textbf{Cartesian-closed categories}. 

\section{Examples of categories with exponential objects}
...
\begin{tikzcd}
  ...
  B^A \times A \arrow[r, "\mathsf{eval}"]                                               & B
  \end{tikzcd}
  \end{center}
  ...
  Then, we define $\mathsf{eval}(f, a)$ to pick the component of the 
  graph $f$ that corresponds to $a \in A$. 

  Now let's show this choice validates the universal property.  Let $\Gamma$ be
  any set and $f$ be a function $\Gamma \times A \to B$, and let $\lambda f$ be
  the morphism that maps $f$ to its graph.
  Clearly the diagram commutes by the definition of a function graph.
  Finally, observe that $\lambda f$ is unique by function extensionality.
\end{proof}

\subsection{Exponents in $\mathsf{Formula}$}

\subsection{Exponents in topological spaces}

\section{Properties of Cartesian-closed categories}
