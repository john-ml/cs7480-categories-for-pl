...
Recall the language \textsc{Calc} from Chapter~\ref{sec:semantics-of-programs}.
\begin{gather}
  \begin{aligned}
...
\item Weakening is precomposition
...
\begin{theorem} \label{thm:calc-products}
  \textsc{Calc} can be interpreted in any category with finite products~(Def~\ref{def:finite-products}).
\end{theorem}
...
it provides a language for working inside of an arbitrary category with finite products.
...
Formally, we say that any category \(\calC\) with finite products has an \textbf{internal language},
and that this language can be used to do work ``internal to \(\calC\)''.

\begin{definition}
  Let \(\calC\) be a category with finite products.
  The \textbf{internal language} of \(\calC\) is
  the extension to \textsc{Calc}
  that adds one primitive type \(\plkw{X}\) for each object \(X\) of \(\calC\)
  and one term former \(\plkw{f}(M)\) for each morphism \(X \xrightarrow{f} Y\),
  with typing rule
  \begin{mathpar}
    \inferrule*
