\chapter{Universal Constructions I: Products}
We've been hinting at the utility of knowing that categories have certain
...
But, what if we want to see if categories other than $\FinSet$ can give a 
...
The diagrammatic form suggests
an order-based intuition for products:
...
morphism from $\dbracket{\Gamma}$ into the special object $\dbracket{B}^{\dbracket{A}}$.
...
\begin{align}
  ...
  = \dbracket{\Gamma} \mor{\langle \lambda \dbracket{M}, \id \rangle} \dbracket{B}^{\dbracket{A}} \times \dbracket{\Gamma}
...
and the application morphism $\app$ need to satisfy the equational
...
\begin{definition}[Universal property of exponential object] \label{def:exponential}
  ...
  An \emph{exponential of \(B\) by \(A\)}
  is a pair \((B^A,\app)\) consisting of
  an object \(B^A\) of \(\calC\)
  and a morphism
  \(\app\) from \(B^A \times A\) to \(B\)
  such that the following holds:
  for any object $\Gamma$ and any morphism \(f : \Gamma \times A \to B\),
...
\chapter{From Categories to Languages}

Consider a variant of \textsc{Calc} from \Cref{sec:semantics-of-programs},
with a unit type and parameterized by an collection of base types \(b\)
and constants \(c\).
\begin{gather}
  \begin{aligned}
  M,N &::= \pllet{x}{M}{N} \mid x \mid \plunit{} \mid \plpair{M}{N} \mid \plfst{M} \mid \plsnd{M} \\
  A,B &::= A \times B \mid \plUnit \mid b
  \end{aligned}
  \tag{\textsc{Calc}}
  \label{lang:calc}
\end{gather}
...
\begin{theorem} \label{thm:calc-products}
  ...
  provided suitable interpretations of each base type \(b\) as an object
  and each constant \(c\) as a morphism.
\end{theorem}
...
\begin{theorem} \label{thm:cccs}
  ...
  lambda calculus.
\end{theorem}
We'll spare you the details; if curious, check out some of the standard references~\citep{lambek1988introduction,crole1993categories}.
