...
These are notes from a course on category theory for programming languages researchers.  I
would say that these notes, and this course, are developed in anger: I have been
...
teach this class and write these notes. This story begins with Lilac~\citep{li2023lilac},
...
are foreshadowing here, but we will return to the Day convolution later on 
...
\chapter{Why Categories?} \label{sec:semantics-of-programs}
...
Consider a tiny programming language
\ref{lang:calc} consisting of let-bindings, pairs, and Booleans:
\begin{gather}
  \begin{aligned}
  M,N &::= \pllet{x}{M}{N} \mid x \mid \pltrue{} \mid \plfalse{} \mid \plite{M}{N}{O} \mid \plpair{M}{N} \mid \plfst{M} \mid \plsnd{M} \\
  A,B &::= A \times B \mid \plBool
  \end{aligned}
  \tag{\textsc{Calc}}
  \label{lang:calc}
\end{gather}
\ref{lang:calc} has a simple type system:
\begin{mathpar}
  \inferrule{~}{\Gamma \vdash \pltrue : \plBool}
  \and
  \inferrule{~}{\Gamma \vdash \plfalse : \plBool}
  \and
  \inferrule
    {
      \Gamma \vdash M : \plBool
      \\
      \Gamma \vdash N : A
      \\
      \Gamma \vdash O : A
    }
    {\Gamma \vdash \plite{M}{N}{O} : A}
  \and
...
\end{mathpar}
Here's a simple transformation one might want to justify: two let-bindings can be interchanged
if their definitions do not mention each other.
\begin{center}
  \(\pllet{x}{M}{\Big( \pllet{y}{N}{\plpair{x}{y}} \Big)}\)\\[0.2em]
  \(\stackrel?=\)\\[0.2em]
  \(\pllet{y}{N}{\Big( \pllet{x}{M}{\plpair{x}{y}} \Big)}\)
\end{center}
Intuitively, this equation holds because
\textsc{Calc} programs have no side effects.
We can justify this intuition by proving that these two programs
have the same meaning. For this, we need to give a
semantics to \textsc{Calc} programs.
...
stating it.  The metalanguage should be a formal language that is
unambiguous to describe and use, because its meaning will be taken for granted
when we interpret programs.  For a simple language like \ref{lang:calc}, a
natural choice is the language of sets and functions.
...
%% We use the \textbf{semantic bracket} $\dbracket{-}$ to define this semantics.
%% To give a semantics of \textsc{Calc} programs, it is first useful to give a
%% semantics of types and contexts.
Types will be interpreted as sets:
\begin{align} \label{eqn:calc-types-in-finset}
  \dbracket{\plBool} &= \{0,1\} \\
  \dbracket{{A}\times{B}} &= \dbracket{A} \times \dbracket{B}
\end{align}
The Boolean type denotes the two-element set \(\{0,1\}\),
and the product type \({A}\pltimes {B}\) denotes the Cartesian product of
the denotations of \(A\) and \(B\).
...
Using this interpretation of types, we can give an interpretation to typing contexts:
\begin{align*}
  \dbracket{\ctxemp} = \{\star\},
  \qquad\qquad
  \dbracket{\Gamma, x : A} =  \dbracket{\Gamma} \times \dbracket{A}
\end{align*}
The empty context \(\ctxemp\) is interpreted as the singleton set \(\{\star\}\),
and context extension by Cartesian product.
Taken together, this interpretation defines \(\llbr{\Gamma}\)
to be the set of \emph{substitutions of shape \(\Gamma\)},
whose elements are functions that map variables \(x\) in \(\Gamma\)
to elements of \(\llbr{\Gamma(x)}\).

Using this interpretation of types and contexts, we can interpret \textsc{Calc} terms
\(\Gamma\vdash M : A\)
as functions
$\dbracket{\Gamma \vdash M : A} : \dbracket{\Gamma} \to \dbracket{A}$:
\marginnote{Notational note: we use ``$\gamma \mapsto v$'' to mean the function that
takes each substitution $\gamma$ to value $v$, and
$\pi_1$ and $\pi_2$ for the two projections out of a Cartesian product.
As usual, we also use $\pi_x$ to magically pick out the component
of an environment \(\gamma\) that corresponds to a given variable \(x\).}
\begin{align*}
  \dbracket{\pltrue{}} &= \gamma \mapsto 1\\
  \dbracket{\plfalse{}} &= \gamma \mapsto 0\\
  \dbracket{\plite{M}{N}{O}} &= \gamma \mapsto \begin{cases}
      \llbr{N} \gamma, & \llbr{M} \gamma = 1 \\
      \llbr{O} \gamma, & \llbr{M} \gamma = 0 \\
    \end{cases}\\
...
\end{align*}
We can use this semantics to validate the reordering from earlier, as
both side of the equation denote the same function.
\begin{fullwidth}
\begin{align*}
...
argument:
\begin{enumerate}
  \item To languages with more interesting kinds of effects, such as
    randomness, nondeterminism, or mutable state;
...
let us first see an example of each of these kinds of generalization.
...
\textsc{TinyPPL} that extends \ref{lang:calc} with a primitive \(\plflip\) that generates random Booleans:
\begin{gather}
  \begin{aligned}
  M,N &\Coloneqq \dots \mid \plflip{} \\
  \end{aligned}
  \tag{\textsc{TinyPPL}}
  \label{lang:tinyppl}
\end{gather}
...
\begin{align*}
  \dbracket{\plflip{}} &= \dbracket{\plBool} \to [0,1] \\
  \dbracket{\plflip{}} &= b \mapsto
  \begin{cases}
    1/2 \quad& \text{if }b=\pltrue\\
    1/2 \quad& \text{if }b=\plfalse\\
...
way of designing these specifications.\footnote{Indeed, the C in OCaml stands for ``categorical''~\citep{cousineau1987categorical}.}
...
properties like ``higher order functions'', a meaning of ``natural numbers'',
 ``logical truth,'' and the ``existence of fixed points''. Category theory gives us a formal language for describing what kind of
...
\item
  ...
  which as a metalanguage supports \emph{very} recursive types.\footnote{%
    For instance, the type \(A = \mathop{\blacktriangleright} \wp(A)\)
    that is isomorphic to its own power set (up to the so-called ``later modality'' \(\blacktriangleright\)).}
...
\item Ordinary continuous probability lives in a \emph{category of measurable spaces},
  and its higher-order generalization lives in \emph{quasi-Borel spaces}.
...
\begin{definition}[Category]
  ...
  \end{itemize}
\end{definition}
This official definition is a bit unwieldy,
so in practice we will use a lot of shorthand.
We say two morphisms \(f,g\) are \emph{composable} if \(\mathsf{dom}(f) = \mathsf{cod}(g)\).
The composition \(\mathsf{comp}(f,g)\) of two composable morphisms will be written \(f \circ g\).
If we ever write \(f \circ g\), it will be tacitly assumed that \(f\) and \(g\) are composable.
Finally, we sometimes omit the subscript from \(\mathsf{id}_X\) when \(X\) is clear from context.
With these abbreviations in mind, the final two laws in the above definition read more comfortably as:
\begin{itemize}
\item \(f \circ (g \circ h) = (f \circ g) \circ h\)
\item \(f \circ \mathsf{id} = f = \mathsf{id} \circ f\).
\end{itemize}
...
\marginnote{Sticklers may raise an eyebrow at
...
\begin{definition}[the category \(\FinSet\) of finite sets] \label{def:finset}
  \sloppy
...
\end{definition}
Again, shorthand helps to make this definition palatable:
we often conflate the tuple \((A,f,B)\) the underlying function \(f\)
when working with morphisms of \(\FinSet\).
...
For instance,
the following is a graph of the negation function \(f : \{T,F\} \to \{T,F\}\)
in the category \(\FinSet\), which could serve as a denotation
of a program like \(x:\plBool \vdash \plite{x}{\plfalse}{\pltrue} : \plBool\).
...
%% The above picture tells us that there \emph{is} a morphism called $f$,
%% but it does not tell us anything about the \emph{content} of $f$.
%% In particular, there's no way to tell what \(f\) will do when given \(T\) or \(F\) as input.
%% To see this, we can draw the \textbf{internal picture} of this category
%% that visualizes the content of its morphisms:\footnote{Not
...
%% Do not confuse the internal picture of a category with its external picture: the internal
...
%% i.e., when they send equal inputs to equal outputs.

%% For instance, we can visualize the identity
%% \(\mathsf{not} \circ \mathsf{not} = \mathsf{id}\)
%% as an equality of internal pictures:
...
%%     % Arrow from F on the left to T on the right
%%     \draw[->, thick] (0.3,-0.3) -- (3.7,0.3) node[midway, below] {$\mathsf{not}$};

%%   % Second circle with T and F dots
%%   \draw[thick] (8,0) circle (1);
%%   \node (T2) at (7.7,0.3) {$\bullet$};
%%   \node at (7.7,0.6) {$T$};
%%   \node (F2) at (8.3,-0.3) {$\bullet$};
%%   \node at (8.3,-0.6) {$F$};

%%     % Arrow from T on the left to F on the right
%%     \draw[->, thick] (3.7,0.3) -- (8.3,-0.3);

%%     % Arrow from F on the left to T on the right
%%     \draw[->, thick] (4.3,-0.3) -- (7.7,0.3) node[midway, below] {$\mathsf{not}$};
%% \end{tikzpicture}
%%   \\[0.5em]
%%   \scalebox{2}{=}
%%   \\[0.5em]
...
%%     % Arrow from T on the left to T on the right
%%     \draw[->, thick] (-0.3,0.3) -- (3.7,0.3);

%%     % Arrow from F on the left to F on the right
%%     \draw[->, thick] (0.3,-0.3) -- (4.3,-0.3) node[midway, below] {$\mathsf{id}$};
%% \end{tikzpicture}
%% \end{center}
...
The definition of category is highly abstract, and categories in the wild can look quite different from \(\FinSet\).
In time, we will see how this level of abstraction allows for
...
But in order to work at this level of abstraction,
it will help to have in mind a few concrete examples.
Broadly speaking, there are three different classes of example
that are handy to have in the back of one's mind when thinking about categories.

\subsection{Categories as graphs}

The first perspective, perhaps most
comfortable to computer scientists, is that a category is a kind of graph.
Objects are vertices and morphisms are edges.
On top of this, categories come with a
...
This provides nice visual intuition, and a large stock of examples.
In fact,
\begin{construction}
