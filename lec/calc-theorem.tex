...
\end{tikzcd}\]
  ...
  We assume given the interpretation of base types,
  with each base type \(b\) denoting some chosen object \(\llbr{b}\) of \(\calC\).
  This interpretation is bootstrapped into one for all types by induction:
  \begin{align}
    \llbr{\plUnit} &= 1 \\
  ...
  and context extension is interpreted via product---again, we are using the same symbols for the abstract categorical definitions
  rather than the set-theoretic ones.
  \begin{align}
    \llbr{\ctxemp} &= 1 \\
  ...
  and \(N\), the object \(\llbr{\Gamma}\) must be
  ...
  for \(\llbr{-}\) to be a function on terms,
